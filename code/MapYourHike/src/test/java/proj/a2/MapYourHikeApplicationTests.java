...
/*
		...
		*/
		...
		user.setUserName("DobnerImola");
		user.setUserPassword("9876");
		if(usersService.checkUser(user)) {
			System.out.println("You are logged in");
			UserAccount existinguser=usersService.findUser(user);
			Profile profile=usersService.findProfile(existinguser);
			System.out.println(profile);
			System.out.println("Search for a trip in Bucegi");
			List<Trip> trip=tripeDao.findAllByMountain("Bucegi");
			System.out.println(trip.get(1));
		}
		else System.out.println("Wrong password or user");
